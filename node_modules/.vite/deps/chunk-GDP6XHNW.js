...
function weierstrassN(CURVE, curveOpts = {}) {
  ...
  const toBytes7 = curveOpts.toBytes || pointToBytes2;
  const fromBytes7 = curveOpts.fromBytes || pointFromBytes;
  ...
  function finishEndo(endoBeta, k1p, k2p, k1neg, k2neg) {
    k2p = new Point4(Fp3.mul(k2p.X, endoBeta), k2p.Y, k2p.Z);
  ...
  class Point4 {
    ...
    static normalizeZ(points) {
      return normalizeZ(Point4, points);
    }
    static fromBytes(bytes) {
      abytes(bytes);
      return Point4.fromHex(bytes);
    }
    /** Converts hash string or Uint8Array to Point. */
    static fromHex(hex) {
      const P = Point4.fromAffine(fromBytes7(ensureBytes("pointHex", hex)));
      P.assertValidity();
      return P;
    }
    /** Multiplies generator point by privateKey. */
    static fromPrivateKey(privateKey) {
      return Point4.BASE.multiply(_normFnElement(Fn, privateKey));
    }
    // TODO: remove
    ...
    negate() {
      return new Point4(this.X, Fp3.neg(this.Y), this.Z);
    }
  ...
  Point4.Fn = Fn;
  const bits = Fn.BITS;
  const wnaf = new wNAF(Point4, curveOpts.endo ? Math.ceil(bits / 2) : bits);
  return Point4;
}
function weierstrassPoints(c) {
  const { CURVE, curveOpts } = _weierstrass_legacy_opts_to_new(c);
  const Point4 = weierstrassN(CURVE, curveOpts);
  return _weierstrass_new_output_to_legacy(c, Point4);
}
...
function ecdsa(Point4, hash, ecdsaOpts = {}) {
  ...
  const { Fp: Fp3, Fn } = Point4;
  const { ORDER: CURVE_ORDER, BITS: fnBits } = Fn;
  const seedLen = getMinHashLength(CURVE_ORDER);
  const lengths = {
    secret: Fn.BYTES,
    public: 1 + Fp3.BYTES,
    publicUncompressed: 1 + 2 * Fp3.BYTES,
  ...
  class Signature {
    ...
    static fromBytes(bytes, format2 = "compact") {
      if (format2 === "compact") {
        const L = Fn.BYTES;
        abytes(bytes, L * 2);
    ...
    recoverPublicKey(msgHash) {
      ...
      const R = Point4.fromHex(concatBytes(pprefix((rec & 1) === 0), x));
  ...
  function getPublicKey2(secretKey, isCompressed = true) {
    return Point4.BASE.multiply(_normFnElement(Fn, secretKey)).toBytes(isCompressed);
  }
  ...
  function getSharedSecret(secretKeyA, publicKeyB, isCompressed = true) {
    ...
    const b = Point4.fromHex(publicKeyB);
  ...
  function sign5(msgHash, secretKey, opts = defaultSigOpts) {
  ...
  function verify5(signature, msgHash, publicKey, opts = defaultVerOpts) {
    ...
    if (format2 === void 0) {
      ...
      if (format2 === "compact" || format2 === "der") {
        ...
        _sig = Signature.fromBytes(ensureBytes("sig", sg), format2);
      } else if (format2 === "js") {
...
function _weierstrass_new_output_to_legacy(c, Point4) {
  const { Fp: Fp3, Fn } = Point4;
...
function weierstrass(c) {
  ...
  const Point4 = weierstrassN(CURVE, curveOpts);
  const signs = ecdsa(Point4, hash, ecdsaOpts);
...
function createHasher3(Point4, mapToCurve, defaults) {
  if (typeof mapToCurve !== "function")
    throw new Error("mapToCurve() must be defined");
  function map(num2) {
    return Point4.fromAffine(mapToCurve(num2));
  }
...
function createSWU(Point4, opts) {
  const map = mapToCurveSimpleSWU2(Point4.Fp, opts);
...
async function sign2(options) {
  ...
  const r = toBigInt2(slice(signature_bytes, 0, 32));
  let s = toBigInt2(slice(signature_bytes, 32, 64));
...
function tower12(opts) {
  ...
  function isValidC(num2, ORDER2) {
    return typeof num2 === "bigint" && _0n8 <= num2 && num2 < ORDER2;
  }
  ...
  const Fp22 = {
    ...
    isValidNot0: (num2) => !Fp22.is0(num2) && Fp22.isValid(num2),
  ...
  const Fp62 = {
    ...
    isValidNot0: (num2) => !Fp62.is0(num2) && Fp62.isValid(num2),
  ...
  const Fp122 = {
    ...
    isValidNot0: (num2) => !Fp122.is0(num2) && Fp122.isValid(num2),
...
var bls12_381_Fr = Field2(bls12_381_CURVE_G1.n, { modOnDecode: true });
...
function signatureG1FromBytes(hex) {
  ...
  const P = Fp.ORDER;
  const Point4 = bls12_381.G1.Point;
  const compressedValue = bytesToNumberBE(value);
  if (infinity)
    return Point4.ZERO;
  const x = Fp.create(compressedValue & Fp.MASK);
  ...
  if (y * _2n7 / P !== aflag)
    y = Fp.neg(y);
  const point = Point4.fromAffine({ x, y });
  point.assertValidity();
  return point;
}
...
function pointG2FromBytes(bytes) {
  ...
  const slc = (b, from14, to) => bytesToNumberBE(b.slice(from14, to));
...
function signatureG2FromBytes(hex) {
  ...
  const Point4 = bls12_381.G2.Point;
...
var Fp2562 = Field(p256_CURVE2.p);
var Fp3842 = Field(p384_CURVE2.p);
var Fp5212 = Field(p521_CURVE2.p);
function createSWU2(field, opts) {
  const map = mapToCurveSimpleSWU(field, opts);
  return (scalars) => map(scalars[0]);
}
var p2563 = createCurve({ ...p256_CURVE2, Fp: Fp2562, lowS: false }, sha256);
var p256_hasher2 = (() => {
  return createHasher(p2563.Point, createSWU2(Fp2562, {
    A: p256_CURVE2.a,
    B: p256_CURVE2.b,
...
var p3842 = createCurve({ ...p384_CURVE2, Fp: Fp3842, lowS: false }, sha384);
var p384_hasher2 = (() => {
  return createHasher(p3842.Point, createSWU2(Fp3842, {
    A: p384_CURVE2.a,
    B: p384_CURVE2.b,
...
var p5212 = createCurve({ ...p521_CURVE2, Fp: Fp5212, lowS: false, allowedPrivateKeyLengths: [130, 131, 132] }, sha512);
var p521_hasher2 = (() => {
  return createHasher(p5212.Point, createSWU2(Fp5212, {
    A: p521_CURVE2.a,
    B: p521_CURVE2.b,
...
*/
//# sourceMappingURL=chunk-GDP6XHNW.js.map
